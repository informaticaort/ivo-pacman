...
Mobile.prototype.checkNextPosition = function(dir){
    ...
    let offset_y = 0;
    if(dir == "left"){
        next_x -= this.velocidad; 
    }
    else if(dir == "right"){
        next_x += this.velocidad; 
    }
    else if(dir == "up"){
        next_y -= this.velocidad; 
    }
    else if(dir == "down"){
        next_y += this.velocidad; 
    }
...
Mobile.prototype.checkNextPositionTry = function(dir){
    ...
    let offset_x = 24; //un desplazamiento para hacer coincidir las rejillas de canvas para el mapa y el canvas para el path
    let offset_y = 0;
    let speed_to_try = this.velocidad; // probamos primero con el mayor desplazamiento posible
...
Mobile.prototype.moveRight = function(){
    ...
    if(can>0){
        ...
        this.mode = "pause";
    if(this.x > PANTALLA_ANCHO) //si entramos en el tuner de la izquierda
        this.x = PANTALLA_COMIENZO;
}
